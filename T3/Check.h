...
*/
bool check_board(int dim_x, int dim_y, char board[dim_x][dim_y]);
/*
...
char board[dim_x][dim_y] - the previosuly generated board
...
/*
checks if there are any penguins left to place by checking if penguins_placed>=2*number_penguins
if false increments penguins_placed by 1
...
returns either 1 (if penguins_placed>=2*number_penguins) or 0 (when it's not)
*/
bool check_placement_coordinates(int dim_x, int dim_y, char board[dim_x][dim_y], int x, int y);
/*
...
*/
bool check_any_valid_moves_left(int dim_x, int dim_y, char board[dim_x][dim_y]);
/*
...
*/
bool check_current_player_has_moves(int dim_x, int dim_y, char board[dim_x][dim_y], int current_player);
/*
...
returns either 1 (when at least 1 penguin can move) or 0 (When no penguins can move)
*/
bool check_if_penguin_can_move(int dim_x, int dim_y, char board[dim_x][dim_y], int penguin);
/*
...
int dim_x, dim_y - board dimensions previously input by player
char board[dim_x][dim_y] - the previosuly generated board
int penguin - the selected penguin

returns either 1 (when it can move) or 0 (when it can't)
*/

bool check_if_move_is_valid(int dim_x, int dim_y, char board[dim_x][dim_y], int x, int y);
/*
